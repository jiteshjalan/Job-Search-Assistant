...
async function callWithFallback(genAI: GoogleGenerativeAI, prompt: string): Promise<string> {
  ...
  for (const modelName of MODEL_CHAIN) {
    try {
      ...
      console.log(`[score-job] model: ${modelName}`);
      return result.response.text();
    } catch (err: unknown) {
      const e = err as { status?: number; message?: string };
      const retryable = e?.status === 429 || e?.status === 404 || e?.status === 503;
      console.warn(`[score-job] ${modelName} failed (${e?.status}): ${e?.message}`);
...

{
  ...
  return {
    ...
    compositeScore,
    dimensions,
    // Extended scoring data — available for future UI use
...
// ---------------------------------------------------------------------------
// POST handler — scores a single job (used by per-card Refresh button)
// ---------------------------------------------------------------------------
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[score-job] Scoring: ${rawJob.company} — ${rawJob.title}`);

    const scoredJob = await scoreOneJob(rawJob, cvText, preferences, genAI);
